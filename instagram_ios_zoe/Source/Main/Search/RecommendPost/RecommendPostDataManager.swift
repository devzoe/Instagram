...
class RecommendPostDataManager {
    ...
    func getRandomFeed(postIdx: Int,delegate: SearchViewController) {
        let url = "\(Constant.BASE_URL)/posts/randFeeds/\(postIdx)"
        let token = UserDefaults.standard.string(forKey: "LoginUserIdentifier") ?? "none"
        print("My Token : ", token)
        
        AF.request(url, method: .get, parameters: nil, encoding: JSONEncoding.default, headers: ["X-ACCESS-TOKEN":token])
            .validate()
            .responseDecodable(of: RecommendFeedResponse.self) { response in
                switch response.result {
                case .success(let response):
                    // 성공했을 때
                    if response.isSuccess, let result = response.result {
                        delegate.didGetRandomFeed(result: result)
                    }
                    // 실패했을 때
                    else {
                        switch response.code {
