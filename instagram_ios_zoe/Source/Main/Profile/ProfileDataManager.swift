...
class ProfileDataManager {
    func getProfileData(delegate: TopProfileViewController) {
        var url = "\(Constant.BASE_URL)/profiles?"
        //내 유저 idx
        let userIdx = UserDefaults.standard.integer(forKey: "UserIdx")
        url += "profileIdx=\(userIdx)&userIdx=\(userIdx)"
    ...
    func getProfileDataInTableView(delegate: ProfileTableViewController) {
        var url = "\(Constant.BASE_URL)/profiles?"
        //내 유저 idx
        let userIdx = UserDefaults.standard.integer(forKey: "UserIdx")
        url += "profileIdx=\(userIdx)&userIdx=\(userIdx)"
